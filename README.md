...
There is intentionally no cross-component ordering. Each component is a
separate ArgoCD Application created directly by the ApplicationSet controller,
and every Application syncs independently and in parallel. ArgoCD sync-wave
annotations only order resources *within* a single Application's sync (or child
Applications under an app-of-apps), so they have no effect on Applications
generated by an ApplicationSet — for that reason they are deliberately not used
here.

Instead, components converge via retry and self-heal: an Application whose
