...
1. Establish a project name.
  ...
  2. For each project, create a Keycloak group (conventionally named `project-<project_name>-edit`; it must match a `name` you list under `groups` in step 3) by submitting a pull request against <https://github.com/CCI-MOC/moc-keycloak/blob/main/groups.tf> or by opening an issue in [moc-issues](https://github.com/CCI-MOC/MOC-issues) and assigning it to someone from the [keycloak-workers] team.
  ...
  3. For each project, create a pull request adding an entry to <https://github.com/CCI-MOC/oac-apps/blob/main/values/oac-prod-infra/oac-prod-workload0/user-projects.yaml>. A minimal entry may look like this:
      ...
      - name: super-duper
        ...
        groups:
        - name: project-super-duper-edit
          role: edit
      ```

      This will create a namespace on the `oac-prod-workload0` cluster named `project-super-duper` and, for each entry under `groups`, a RoleBinding granting that Keycloak group the given ClusterRole (`edit`, `view`, ...) in the namespace. Every project must list at least one group, or the chart fails to render. If an entry omits `name`, it defaults to `project-<project_name>-<role>`. This will apply default quotas to the namespace (see <https://github.com/CCI-MOC/oac-apps/blob/main/charts/user-projects/values.yaml> for the default values). You can override these defaults when defining the project:
